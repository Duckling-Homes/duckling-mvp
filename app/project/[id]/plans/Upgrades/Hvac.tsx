...
import { Add, Clear, DeviceThermostat } from "@mui/icons-material"
...
const Hvac: React.FC<HvacProps> = ({ catalogue, plan, projectId }) => {
  ...
  useEffect(() => {
    setItems(
      catalogue.filter(item => item.category === "HVAC")
    )
  }, [catalogue])
  ...
  function addWorkItem(item: CatalogueItem) {
    ...
    ModelStore.addCatalogItem(plan.id, newWorkItem,'Hvac')
  }
  ...
  function removeWorkItem(itemCustomId: string) {
    ...
    setWorkItems(newWorkItemsList)
    ModelStore.removeCatalogItem(plan.id, itemCustomId,'Hvac')
  ...
  const handlePropertyChange = (customId: string, propertyName: string, newValue: any) => {
    ...
    if (propertyName === 'quantity') {
      ModelStore.updateCatalogItemProperty(plan.id, customId, 'Hvac', newValue, 'quantity');
    } else if (propertyName === 'customName') {
      ModelStore.updateCatalogItemProperty(plan.id, customId, 'Hvac', newValue, 'customName');
    }
  };
  ...
  function renderWorkItems() {
    return workItems.map(item => (
      <React.Fragment key={item.customId}>
        <Divider />
        <div className="hvac__workItem" key={item.customId}>
          <div className="hvac__workItemHeader">
            <span>{item.subcategory}</span>
            <span>Estimated Cost: ${item.quantity * item.basePricePer}</span>
          </div>
          <div className="hvac__workItemContent">
            <TextField
              label="Name"
              placeholder="Name"
