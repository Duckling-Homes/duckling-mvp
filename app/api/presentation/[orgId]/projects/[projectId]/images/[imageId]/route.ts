...
export async function GET(
  ...
  try {
    const { orgId, projectId, imageId } = context.params
    if (!(await isImageInOrganization(imageId, orgId))) {
    ...
    if (!(await isImageInProject(imageId, projectId))) {
      return new NextResponse('Not found', {
        status: 404,
        headers: {
          'Content-Type': 'application/json',
    ...
    const s3Key = constructS3ImageKey(orgId, imageType, imageId)
