...
const main = async (): Promise<void> => {
  ...
  if (args.length < 3) {
    ...
    console.error("Usage: ./scripts/onboardOrganization.ts <databaseEndpoint> <orgName> <email1,email2,...>");
    console.error("Example: ./scripts/onboardOrganization.ts 'postgresql://admin:admin@localhost:5432/db' 'NewOrg' 'user1@example.com,user2@example.com'");
    process.exit(1);
  }
  ...
  if (existingOrg) {
    ...
    console.log(`Created new organization '${orgName}' with ID: ${existingOrg.id}.`);
  }
  ...
  for (const email of emails) {
    ...
    console.log(`User with email '${email}' has been created and associated with organization '${orgName}'.`);
  }
